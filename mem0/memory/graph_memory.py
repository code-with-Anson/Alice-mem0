...
class MemoryGraph:
    ...
    # 原来的def _retrieve_nodes_from_data方法被注释掉了
    # 接下来创建一个为中文提示词优化的方法
    ...
    def _retrieve_nodes_from_data(self, data, filters):
        ...
        # 添加明确的中英双语提示词
        system_content = f"""You are a smart assistant who understands entities and their types in a given text. 
        ...
        如果文本是中文，请提取所有实体和它们的类型。实体类型包括：人物、地点、组织、概念等。
        如果用户消息中包含自我引用，如"我"、"我的"等，请使用 {filters['user_id']} 作为源实体。
        提取所有实体，不要回答问题本身。

        返回的JSON格式必须完全正确，不要添加任何额外的解释或注释。

        示例输入（中文）：
        "爱丽丝是一名游戏开发部的部员，她喜欢玩游戏。"

        示例输出（工具调用）：
        {{
          "entities": [
            {{"entity": "爱丽丝", "entity_type": "人物"}},
            {{"entity": "游戏开发部", "entity_type": "组织"}},
            {{"entity": "部员", "entity_type": "职业"}},
            {{"entity": "玩游戏", "entity_type": "爱好"}}
          ]
        }}
        """
    ...
    def _establish_nodes_relations_from_data(self, data, filters, entity_type_map):
        ...
        # 添加中英双语提示
        if self.config.graph_store.custom_prompt:
            custom_prompt_with_chinese = f"""4. {self.config.graph_store.custom_prompt}

            如果文本是中文，请识别实体之间的关系。例如：
            - "爱丽丝是游戏开发部的部员" 表示 爱丽丝 [是] 部员，爱丽丝 [属于] 游戏开发部
            - "才羽桃井喜欢打游戏" 表示 才羽桃井 [喜欢] 打游戏
            ...
            messages = [
                {
                    ...
                    "content": EXTRACT_RELATIONS_PROMPT.replace(
                        "USER_ID", filters["user_id"]
                    ).replace("CUSTOM_PROMPT", custom_prompt_with_chinese),
                },
                {"role": "user", "content": data},
            ]
        else:
            chinese_prompt = f"""如果文本是中文，请识别实体之间的关系。
            实体列表：{list(entity_type_map.keys())}

            请确定这些实体之间的关系，如：[是]、[属于]、[工作于]、[喜欢]、[创建]等。
    ...
    def _get_delete_entities_from_search_output(self, search_output, data, filters):
        ...
        # 添加中文提示
        chinese_prompt = """
        如果用户的消息是中文，请同样分析哪些实体关系需要被删除。

        例如，如果存储的关系是"爱丽丝 [喜欢] 咖啡"，但用户现在说"爱丽丝讨厌咖啡"，那么应该删除原来的关系。

        请确保返回的JSON格式完全正确，不添加任何额外的解释或注释。
        """
